                    ...
                    if to_log:
                        for i in local_var_dump:
                            ...
                            if len(str(local_var_dump[i])) > max_var_str_len:    
                                local_var_dump[i] = str(type(local_var_dump[i])) + ' ' + str(local_var_dump[i])[:max_var_str_len] 
...
class Resilient(object):
    ...
    def __enter__(self):
        pass
    def __exit__(self, exc_type, exc_value, traceback, _var_dump=None, exc_info=None):
            ...
            if self.to_log:
                for i in final_var_dump:
                    ...
                    if self.blacklist_var and i in self.blacklist_var:
                        continue
                    if len(str(final_var_dump[i])) > self.max_var_str_len:    
                        final_var_dump[i] = str(type(final_var_dump[i])) + ' ' + str(final_var_dump[i])[:self.max_var_str_len] 
...
def _determine_sleep_time(current_time, min_time, max_time):
    ...
    # add jitter of 5%
    current_time += random.randint(round(-50 * current_time), round(50 * current_time)) / 1000
    return current_time
